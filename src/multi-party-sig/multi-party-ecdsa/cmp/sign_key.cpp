...
bool SignKey::ToProtoObject(safeheron::proto::multi_party_ecdsa::cmp::SignKey &sign_key) const {
    bool ok = true;
    string str;
    ...
    for (size_t i = 0; i < remote_parties_.size(); i++) {
        safeheron::proto::multi_party_ecdsa::cmp::Party remote_party;
        ok = remote_parties_[i].ToProtoObject(remote_party);
        if (!ok) return false;
        safeheron::proto::multi_party_ecdsa::cmp::Party *new_party = sign_key.add_remote_parties();
...
bool SignKey::FromProtoObject(const safeheron::proto::multi_party_ecdsa::cmp::SignKey &sign_key) {
    bool ok = true;
...
bool SignKey::ValidityTest() const {
    bool ok = true;
    const curve::Curve *secp256k1 = curve::GetCurveParam(local_party_.X_.GetCurveType());
    ...
    // Step1: check g^u == y
    if (secp256k1->g * local_party_.x_ != local_party_.X_ ) return false;
    ...
    vector<BN> l_arr;
    Polynomial::GetLArray(l_arr, BN::ZERO, share_index_arr, secp256k1->n);

    CurvePoint pub = local_party_.X_ * l_arr[share_index_arr.size() - 1];
    for (size_t i = 0; i < remote_parties_.size(); ++i) {
...
typedef safeheron::proto::multi_party_ecdsa::cmp::SignKey ProtoObject;
