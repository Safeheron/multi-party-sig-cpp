...
bool Round0P2PMessage::ToProtoObject(safeheron::proto::multi_party_ecdsa::cmp::sign::Round0P2PMessage &message) const {
    bool ok = true;
    ...
    safeheron::proto::PailEncRangeProof_V2 t_proof;
    ok = psi_0_ij_.ToProtoObject(t_proof);
    if (!ok) return false;
    message.mutable_psi_0_ij()->CopyFrom(t_proof);

    return true;
}

bool Round0P2PMessage::FromProtoObject(const safeheron::proto::multi_party_ecdsa::cmp::sign::Round0P2PMessage &message) {
    bool ok = true;
    ...
    ok = psi_0_ij_.FromProtoObject(message.psi_0_ij());
    if (!ok) return false;
...
typedef safeheron::proto::multi_party_ecdsa::cmp::sign::Round0P2PMessage ProtoObject;
