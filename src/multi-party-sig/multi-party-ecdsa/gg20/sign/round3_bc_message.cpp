...
bool Round3BCMessage::ToProtoObject(safeheron::proto::multi_party_ecdsa::gg20::sign::Round3BCMessage &message) const {
    bool ok = true;
    ...
    if (!ok) return false;
    message.mutable_gamma()->CopyFrom(point);

    return true;
}

bool Round3BCMessage::FromProtoObject(const safeheron::proto::multi_party_ecdsa::gg20::sign::Round3BCMessage &message) {
    bool ok = true;

    blind_factor_ = BN::FromHexStr(message.blind_factor());

    ok = Gamma_.FromProtoObject(message.gamma());
    if (!ok) return false;
...
typedef safeheron::proto::multi_party_ecdsa::gg20::sign::Round3BCMessage ProtoObject;
