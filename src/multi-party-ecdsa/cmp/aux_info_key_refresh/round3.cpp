...
bool Round3::ReceiveVerify(const std::string &party_id) {
    ...
    p2p_message_arr_[pos].psi_.SetSalt(ctx->rho_);
    ...
    p2p_message_arr_[pos].phi_ij_.SetSalt(ctx->rho_);
    ...
    p2p_message_arr_[pos].pi_.SetSalt(ctx->rho_);
    ...
    p2p_message_arr_[pos].psi_ij_.SetSalt(ctx->rho_);
...
bool Round3::ComputeVerify() {
    ...
    if(iter0 == ctx->local_party_.map_party_id_x_.end()) {
        ctx->PushErrorCode(1, __FILE__, __LINE__, __FUNCTION__, "iter0 == ctx->local_party_.map_remote_party_id_X_.end()");
        return false;
    }
    BN x = sign_key.local_party_.x_;
    // share of offset from local party
    x = (x + iter0->second) % curv->n;
    for (const auto & remote_party : ctx->remote_parties_) {
        // share of offset from remote party
    ...
    for (size_t i = 0; i < sign_key.remote_parties_.size(); ++i) {
        ...
        if(iter1 == ctx->local_party_.map_party_id_X_.end()) {
            ctx->PushErrorCode(1, __FILE__, __LINE__, __FUNCTION__, "ctx->local_party_.map_remote_party_id_X_.find(current_party_id)");
            return false;
        }
        X += iter1->second;
        for (size_t j = 0; j < ctx->remote_parties_.size(); ++j) {
            ...
            if(iter2 == remote_party.map_party_id_X_.end()) {
                ctx->PushErrorCode(1, __FILE__, __LINE__, __FUNCTION__, "remote_party.map_party_id_X_.find(current_party_id)");
                return false;
            }
            X += iter2->second;
        }
